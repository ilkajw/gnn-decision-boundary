import json
import math
from config import DATASET_NAME, ROOT, ANALYSIS_DIR, MODEL
from index_sets_utils import build_index_set_cuts, cut_pairs, graphs_correctly_classified, pairs_within

if __name__ == "__main__":

    with open(f"{ROOT}/{DATASET_NAME}/test/{DATASET_NAME}_paths_with_target_graph_inserted.json", "r") as f:
        raw = json.load(f)
    ...
    with open(f"{ANALYSIS_DIR}/{DATASET_NAME}_target_graph_insertions_per_idxset.json", "w") as f:
