...
A k-fold cross validation training is defined on training sets augmented with labeled path graphs. For each fold, graphs from paths between a graph pair from the training split are added to the training split.
...
and with the target graph's class afterward
...
In `config.py`, define the settings for analysis and the training on path graph-augmented data:
...
you can start training on the augmented dataset afterward.

If you only want to run certain analysis function, run `run_graph_creation_pipeline.py`, then `precalcs.py` 
and afterward the analysis file you like (`07_xxx.py` to `11_xxx.py`).

If you only want to run the training on augmented training data, run `run_graph_creation_pipeline.py` first. Then, run
`training_augmented.py`.
...
#### 08_path_length_stats_by_num_flips.py
...
Files ``12_plot_num_flips_histogram.py``, ``13_plot_flip_order_distr.py``, 
``14_plot_flips_distr_with_ops.py`` and ``15_plot_op_heatmaps.py`` plot the analysis results and save plots to
``<ANALYSIS_DIR>/plots/``.
